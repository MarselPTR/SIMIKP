...
import { mockTugasPetugas } from "../../lib/mock-data";
...
const PetugasDashboardPage = () => {
  ...
  const userTasks = mockTugasPetugas.filter((t) => t.bidang === user?.bidang);
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-xl border border-gray-200 px-6 py-5 shadow-sm">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Berikut adalah daftar tugas dan aktivitas kamu di bidang <strong>{user?.bidang}</strong> hari ini.
        </p>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-gray-500">Menampilkan bidang: {user?.bidang}</span>
        </div>
