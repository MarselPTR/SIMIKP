...
const BankKontenPage = () => {
  ...
  const { data: folders = [], isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => (await apiFetch<{ data: any[] }>("/productions/bank-konten")).data,
  });
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      return apiFetch("/productions/bank-konten/upload", {
        method: "POST",
    ...
    onSuccess: () => {
      ...
      setUploadFile(null);
      addToast("File berhasil diunggah dan tersimpan ke server", "success");
    },
    onError: (err: any) => addToast(err.message || "Gagal mengunggah file", "error"),
  });
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            <Button variant="default" onClick={handleUploadSubmit} isLoading={uploadMutation.isPending}>Upload</Button>
          </div>
        </div>
      </Dialog>
    </div>
