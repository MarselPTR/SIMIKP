...
const VideoPanel = ({ assignments }: { assignments: any[] }) => {
  ...
  const videoBars = [
    { label: "Tugas Baru", segments: [{ key: "belum", v: belum }] },
    { label: "Sedang Dikerjakan", segments: [{ key: "liputan", v: diproses }] },
    { label: "Finis", segments: [{ key: "finis", v: selesai }] },
  ];
...
const DesainerPanel = ({ assignments }: { assignments: any[] }) => {
  ...
  const data = assignments.filter((a) => ["Infografis", "Banner", "Logo", "Poster"].includes(a.contentType));
  const antrean = data.filter((a) => a.status === "BELUM" || a.status === "ASSIGNED");
  const diproses = data.filter((a) => !["BELUM", "ASSIGNED", "SELESAI", "COMPLETED"].includes(a.status));
  const selesai = data.filter((a) => a.status === "SELESAI" || a.status === "COMPLETED");

  const getItems = (arr: any[]) => arr.slice(0, 2).map((a) => a.activityTitle);

  const desainerColumns = [
    { label: "ANTREAN", count: antrean.length, color: "#9ca3af", bg: "bg-gray-100", text: "text-gray-700", items: getItems(antrean) },
    { label: "DIPROSES", count: diproses.length, color: "#f59e0b", bg: "bg-amber-100", text: "text-amber-700", items: getItems(diproses) },
    { label: "SELESAI", count: selesai.length, color: "#22c55e", bg: "bg-emerald-100", text: "text-emerald-700", items: getItems(selesai) },
  ];
  return (
...
const DashboardPage = () => {
  ...
  const { data: kegiatanData } = useQuery({ queryKey: ["kegiatan"], queryFn: async () => (await apiFetch<{ data: MockKegiatan[] }>("/activities")).data });
  const kegiatanList = kegiatanData ?? [];

  const { data: assignmentsData } = useQuery({ queryKey: ["assignments-dash"], queryFn: async () => (await apiFetch<{ data: any[] }>("/assignments")).data });
