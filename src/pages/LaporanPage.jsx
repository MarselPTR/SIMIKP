...
const LaporanPage = () => {
  const { data: kegiatan }  = useQuery({ queryKey: ['kegiatan'],  queryFn: api.kegiatan.getAll });
  const { data: produksi }  = useQuery({ queryKey: ['produksi'],  queryFn: api.produksi.getAll });
  const { data: publikasi } = useQuery({ queryKey: ['publikasi'], queryFn: api.publikasi.getAll });
